function [eh,ev] = RefThreeConceal(incidence,Param)
% 利用等效多层传输线模型，计算三层介质隐身目标的发射率
% 输入：入射角incidence
...
jd4 = Param.Target.layer3_target_permittivity;          % 涂层3介电常数，雷达18.4-2.57i
jd5 = -inf.*1i;
...
cd4 = Param.Target.layer3_target_permeability;          % 涂层3磁导率，雷达1.03-0.72i
cd5 = 1;
...
thickness3 = Param.Target.layer3_coat_thickness;             % 涂层3厚度（单位m），雷达0.0004
...
incidence_5 = acosd(sqrt(1-((sind(incidence)).^2)./(cd5.*jd5)));
...
Z5 = sqrt(cd5./jd5).*secd(incidence_5);
...
r4 = (2.*pi./Wavelength).*(sqrt(cd4.*jd4));

Re3 = (R3+R4.*exp(-2i.*thickness3.*r4.*secd(incidence_4)))./(1+R3.*R4.*exp(-2i.*thickness3.*r4.*secd(incidence_4)));    % 化简式
Re2 = (R2+Re3.*exp(-2i.*thickness2.*r3.*secd(incidence_3)))./(1+R2.*Re3.*exp(-2i.*thickness2.*r3.*secd(incidence_3)));  % 化简式
...
Z5 = sqrt(cd5./jd5).*cosd(incidence_5);
