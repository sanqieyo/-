function [TB,e_tg,count,target_area,TB_target] = GetRadiationTemp(Param)
%本函数用于计算复杂场景的亮温分布

y_list = Param.Observe.horizon_scale;           % 横向扫描序列
z_list = Param.Observe.vertical_scale;          % 纵向扫描序列

target_point = Param.Target.target_point;       % 原始坐标系中的目标节点坐标
...
distances = zeros(length(z_list),length(y_list));  
e_tg = TB;
count = TB;
...
%             polar_vector_scan = ph*sind(polar_angle)+pv*cosd(polar_angle);    
            ...
            if y_loc<=y_max&&y_loc>=y_min&&z_loc<=z_max&&z_loc>=z_min
                [TB1(1,n),count1(1,n),e_tg1(1,n)] = FindFacet(inc_vector,observe_position,Param,polar_vector_scan);       % 计算像素点对应目标面元的亮温值
            end
