function [TB,trace_count,e_tg] = AccRadiationTrace(facet_num,inc_vector,inc_node,trace_count,polar_vector,Param, screening_target_rank, screening_normal_vector, screening_rank_point)
% 本函数用于复杂场景的亮温追踪
...
[eh_tg,ev_tg] = RefSingleConceal(inc_theta,Param);         % 入射角为inc_theta，计算入射到飞机目标表面时发射率的水平极化和垂直极化分量
...
T = Param.Target.sea.target_temp(facet_num);            % 目标面元温度
...
if isempty(facet_list4)         % 如果不存在相交面元，由入射向量直接计算面元亮温
 ...
 if counter>0       % 判断交点是否存在  
     ...
     [TB,trace_count,~] = AccRadiationTrace(facet_list5(facet_num),re_vector(facet_num,:),re_node(facet_num,:),trace_count,polar_vector,Param,screening_target_rank, screening_normal_vector, screening_rank_point);
     TB = e_tg*T+(1-e_tg)*TB;
     % 对相交面元进行亮温追踪，返回亮温值
 else               % 不存在交点
     TB = e_tg*T+(1-e_tg)*GetEnvironRadiation(polar_vector,90-zenith_theta,Param,ref_vector);
