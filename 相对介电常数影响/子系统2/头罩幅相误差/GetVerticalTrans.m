% 该函数计算头罩的垂直极化传输系数和相位延迟，输入theta单位为°
function [ trans_v, phase_delay_v ] = GetVerticalTrans( theta,d ,epsilon,delta,f0)
%% 基本参数设置
com_epsilon = epsilon * (1 - 1j * delta);
theta_rad = theta * pi /180;
lambda = 3e8/f0;

com_admittance = sqrt(com_epsilon-(sin(theta_rad)^2))/cos(theta_rad); %垂直极化复导纳
...
phase_delay_v = IPD;
phase_delay_v = phase_delay_v*pi/180;
