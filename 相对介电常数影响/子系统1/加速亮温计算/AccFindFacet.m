function [TB,trace_count,e_tg] = AccFindFacet(inc_vector,inc_node,Param,polar_vector_scan, screening_target_rank, screening_normal_vector, screening_rank_point)
% 本函数用于复杂场景的消隐计算
...
% rank_point = Param.Target.rank_point;                   % 导入按面元顶点排列（x,y,z）坐标
% normal_vector = Param.Target.normal_vector;             % 导入面元法向量
% target_rank = Param.Target.target_rank;                 % 导入面元节点信息
target_point = Param.Target.sea.target_point;               % 导入节点坐标信息
...
if isempty(facet_list4)     % 如果不存在相交面元
 ...
 if counter>0           % 判断交点是否存在
     ...
     [TB,trace_count,e_tg] = AccRadiationTrace(facet_list5(facet_num),re_vector(facet_num,:),re_node(facet_num,:),trace_count,polar_vector_scan,Param, screening_target_rank, screening_normal_vector, screening_rank_point);
